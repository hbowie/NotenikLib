//
//  DatePickedValue.swift
//  NotenikLib
//
//  Created by Herb Bowie on 4/6/25.
...
/// This is very similar to DateTimeValue, with the difference that  a DatePickedValue
/// will be blank until explicitly set.
public class DatePickedValue: StringValue {
    ...
    public func format(with: String) -> String {
        if dateAndTime == nil || value.isEmpty {
            return ""
        } else if with.hasPrefix("yyyy-MM-ddT") {
    ...
    static func == (lhs: DatePickedValue, rhs: DatePickedValue) -> Bool {
        return lhs.dateAndTime == rhs.dateAndTime
    }
    
    /// See which of these objects should come before the other in a sorted list
    static func < (lhs: DatePickedValue, rhs: DatePickedValue) -> Bool {
        if lhs.dateAndTime == nil {
            if rhs.dateAndTime == nil {
                return false
            } else {
                return true
            }
        } else if rhs.dateAndTime == nil {
            return false
        } else {
            return lhs.dateAndTime! < rhs.dateAndTime!
        }
