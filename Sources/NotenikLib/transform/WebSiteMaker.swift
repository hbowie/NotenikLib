...
//  NotenikLib
//
//  Created by Herb Bowie on 7/23/22.
//
...
public class WebSiteMaker {
    ...
    public init?(input: URL, output: URL) {
        ...
        guard FileUtils.ensureFolder(forURL: siteFolder) else { return nil }
        
        imagesFolder = siteFolder.appendingPathComponent(imagesFolderName, isDirectory: true)
        ...
        // Prepare the Display parameters.
        ...
        headerFile = URL(fileURLWithPath: headerFileName, relativeTo: siteFolder).appendingPathExtension(headerFileExt)
    ...
    public func generate() -> Int {
        ...
        logInfo(msg: "\(filesWritten) files written to \(siteFolder)")
        return filesWritten
    }
    ...
    func deleteOldFiles() {
        ...
        filesDeleted = 0
        do {
            let contents = try fm.contentsOfDirectory(at: siteFolder,
            ...
            communicateError("Could not read directory at \(siteFolder)")
        }
        logInfo(msg: "\(filesDeleted) files deleted from \(siteFolder)")
    ...
    func generateMimetype() {
        ...
        mimetypeFile = URL(fileURLWithPath: mimetypeFileName, relativeTo: siteFolder)
    ...
    func generateCSS() {
        ...
        cssFolder = siteFolder.appendingPathComponent(cssFolderName, isDirectory: true)
    ...
    func generate(note: Note) {
        ...
        if firstPage && !epub {
            let indexURL = URL(fileURLWithPath: "index", relativeTo: siteFolder).appendingPathExtension(htmlFileExt)
